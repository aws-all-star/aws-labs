...
(async function () {
  ...
  if (db) {
    ...
    app.get("/players/top/:number", async (req, res) => {
      ...
      const cursor = await players.find({}).sort({ G: -1 }).limit(number).project({ _id: 0, "Player Name": 1, Team: 1, G: 1 });
      const results = await cursor.toArray();
      res.json(results);
    });
    ...
    app.get("/players/team/:teamName", async (req, res) => {
      ...
      const results = await cursor.project({ _id: 0, "Player Name": 1, Team: 1, G: 1 }).toArray();
      res.json(results);
    });
    //list all team names
